  ...
  cards: [
...

    {
      ...
      blocks: [
        ...
        { t: "note", kind: "warn", title: "windows.consoles / windows.cmdscan skip unsupported OS versions", text: "Both recover the console screen/input buffer — which is where a pasted command lands, not just a typed one — but they hard-code which Windows versions they know how to parse and raise <code>NotImplementedError</code> on anything outside that list (Windows 7 / NT 6.1 build 7601 included, as of vol3 2.28.0). No flag works around this; it's the plugin, not the image." },
        { t: "cmd", label: "fallback: dump conhost.exe (owns the buffer, not the shell) and grep both encodings", code: "vol -f memdump.raw windows.memmap --dump --pid <conhost-pid>\nstrings    pid.<pid>.dmp | grep -iE 'iex|invoke-expression|frombase64|alias|-enc'\nstrings -el pid.<pid>.dmp | grep -iE 'iex|invoke-expression|frombase64|alias|-enc'\n# or turn it into a CSV for vol-triage.html instead of reading scrollback:\npython3 strings2csv.py pid.<pid>.dmp --pid <pid> --pattern 'iex|alias|frombase64' > out.csv" },
        { t: "note", kind: "info", title: "picking which conhost", text: "A console's conhost.exe and the shell it hosts share a <b>CreateTime</b> — match them in <code>pstree</code> first rather than guessing. A PowerShell process spawned from an existing <code>cmd.exe</code> window usually reuses that cmd's console/conhost rather than getting its own." },
      ],
    },
